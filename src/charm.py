...
class KratosCharm(CharmBase):
    ...
    def _get_active_flags(self) -> FeatureFlags:
        feature_flags = []
        if not self.config_file["selfservice"] or not self.config_file["selfservice"]["methods"]:
            return feature_flags

        methods = self.config_file["selfservice"]["methods"]

        if methods["password"] and methods["password"]["enabled"]:
            feature_flags.append("password")

        if methods["webauthn"] and methods["webauthn"]["enabled"]:
            feature_flags.append("webauthn")

        if methods["totp"] and methods["totp"]["enabled"]:
            feature_flags.append("totp")

        if methods["lookup_secret"] and methods["lookup_secret"]["enabled"]:
            feature_flags.append("backup_codes")

        if len(self.external_idp_requirer.get_providers()) > 0:
