...
class TestSecrets:
    ...
    def test_set_with_secret_found(self, mocked_model: MagicMock, secrets: Secrets) -> None:
        secrets[COOKIE_SECRET_LABEL] = {COOKIE_SECRET_CONTENT_KEY: "cookie"}

        mocked_model.get_secret.assert_called_once_with(label=COOKIE_SECRET_LABEL)
        mocked_model.app.add_secret.assert_not_called()

    def test_set_with_secret_not_found(self, mocked_model: MagicMock, secrets: Secrets) -> None:
        mocked_model.get_secret.side_effect = SecretNotFoundError
